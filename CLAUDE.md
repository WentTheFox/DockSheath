...
- `Sources/DockOverlayKit/OverlayWindow.swift` — the actual overlay window, an `NSPanel` with `.nonactivatingPanel` (not a plain `NSWindow`) so a taskbar click never activates DockSheath itself — a normal activating window would fire `NSWorkspace.didActivateApplicationNotification` for DockSheath on every click, which used to race `RunningWindowsStripView`'s button-rebuild-on-activation and destroy the very button being clicked mid-gesture, silently swallowing the first click. Its level is resolved at runtime via `CGWindowLevelForKey(.dockWindow) + 1` (never hardcoded — the raw level can shift across macOS versions).
...
**Gotcha this has already caused confusion over**: `security find-identity -v -p codesigning` (what the lookup above uses) only lists identities that pass a full trust-chain check for code signing — a self-signed root certificate is *not* trusted for that purpose right after creation, so it silently fails to match by name until you open it in Keychain Access, expand "Trust", and set "Code Signing" to "Always Trust". The script distinguishes "certificate doesn't exist" (checked via `security find-certificate -c`, no trust check) from "certificate exists but isn't trusted yet" and prints the matching instructions for each case, since the fix and the symptom otherwise look identical (same name, still "not found").
